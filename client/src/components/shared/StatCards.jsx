      ...
      <Card className="rounded-2xl border-slate-800 bg-slate-900/70 shadow-sm shadow-black/10 transition hover:border-cyan-400/30 hover:bg-slate-900">
        <CardContent className="p-3">
          <div className="mb-2 flex items-center justify-between">
            <div className="flex h-6 w-6 items-center justify-center rounded bg-slate-800 ring-1 ring-slate-700/70">
              <Icon className="h-3.5 w-3.5 text-cyan-300" />
            </div>
            <span className="rounded bg-emerald-400/10 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wider text-emerald-300 ring-1 ring-emerald-400/10">{status}</span>
          </div>
          <p className="text-[11px] font-medium text-slate-400">{title}</p>
          <h3 className="mt-0.5 text-xl font-bold leading-none text-white">{value}</h3>
          <p className="mt-2 line-clamp-1 border-t border-slate-800/50 pt-1.5 text-[10px] text-slate-500">{note}</p>
        </CardContent>
      </Card>
    </motion.div>
  );
